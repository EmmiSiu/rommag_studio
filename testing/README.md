...
- `npm run test:stage10`: mocked Audio 8D motion flow when a frontend URL is available.
...
- `npm run perf:pipeline`: musical-analysis fixtures, pipeline readiness checks and benchmark instructions.
...
or `API_URL` is set. Stage 9 and Stage 10 use Playwright route mocks for auth,
stems and WAV files, so they need only a running frontend.
